# Gui tutorial
# Gui - En f rste runde�
# F lg skridtene i programmet nedenfor meget n je, s du� � �
# forst r hvad der sker.Lav sm ting om. S du kan se pr cis� � � �
# hvor du er. Osv.
# Og derved f r et f rste indtryk af hvordan der tilf jes� � �
...
window.mainloop()
#The last line which calls mainloop function, this function calls the endless loop
#of the window,
...
# Then we will set its position on the form using the grid function and give it
...
#Let s start by adding the button to the window, the button is created and added to�
#the window the same as the label:
window = Tk()
...
#First, we will write the function that we need to execute when the button is
#clicked:
def clicked():
