...
bullet_speed = 50
bullet_cooldown = 500
bullet_lastFired = pygame.time.get_ticks()
...
def addNewEnemy():
    enemyPos = [random.randint(100, 700), random.randint(100, 500)]
    #pygame.draw.circle(screen, RED, (int(enemyPos[0]), int(enemyPos[1])), 200)
    enemies.append(enemyPos)
...
def main():
    ...
    global player_angle, bullet_lastFired
    ...
    while running:
        ...
        if keys[pygame.K_SPACE]:
            if (pygame.time.get_ticks() - bullet_lastFired) > bullet_cooldown:
